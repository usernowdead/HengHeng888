...
async function handleEasyslipImage(request: NextRequest) {
    try {
        ...
        if (!file) {
            return NextResponse.json({
                success: false,
                message: 'กรุณาอัปโหลดไฟล์ภาพสลิป'
            }, { status: 400 });
        }
        ...
        console.error('EasySlip verify image error:', error);
...
export const POST = withCSRFSecurity(handleEasyslipImage);
