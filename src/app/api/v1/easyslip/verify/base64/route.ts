...
async function handleEasyslipBase64(request: NextRequest) {
    try {
        ...
        // Parse request body
        const { image, checkDuplicate } = await request.json();

        if (!image) {
            return NextResponse.json({
                success: false,
                message: 'กรุณาส่งข้อมูลภาพในรูปแบบ Base64'
            }, { status: 400 });
        }

        // Call EasySlip API
        const response = await axios.post(
            `${EASYSLIP_API_URL}/verify`,
            {
                image: image,
        ...
        console.error('EasySlip verify base64 error:', error);
...
export const POST = withCSRFSecurity(handleEasyslipBase64);
