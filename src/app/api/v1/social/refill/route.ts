...
async function handleSocialRefill(request: NextRequest) {
    try {
        ...
        // Request refill from external API
        try {
            ...
            return NextResponse.json({
                ...
                data: {
                    ...
                    refill: response.data.refill || response.data,
                    message: 'ส่งคำขอ refill สำเร็จ'
                }
            });
        } catch (error: any) {
            console.error('Error requesting refill from external API:', error);
            return NextResponse.json({
                success: false,
                message: error.response?.data?.error || 'ไม่สามารถส่งคำขอ refill ได้'
            }, { status: 500 });
        }
    } catch (error: any) {
        ...
        return NextResponse.json({
            success: false,
            message: error.message || 'เกิดข้อผิดพลาดในการส่งคำขอ refill'
        }, { status: 500 });
    }
}

// Export with CSRF protection and rate limiting
export const POST = withCSRFSecurity(handleSocialRefill);
